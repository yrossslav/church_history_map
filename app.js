...
const NOFLOW=new Set(['lutc>ierem','refc>lukaris','qua>dukhob']);   // боковые межтрадиционные влияния (не тянут родословную в чужое дерево)
...
function showTree(idset,title,focal,mode){card.style.display='none';   // прячем карточку при открытии окна ветви
  treeHist=treeHist.slice(0,treeIdx+1);
...
function changelogHtml(){return (CONTENT.changelog||[]).map(v=>
  '<p><b>'+esc(v.date||'')+'</b>'+(v.note?' · '+esc(v.note):'')+'</p><ul>'+(v.items||[]).map(it=>'<li>'+esc(it)+'</li>').join('')+'</ul>'
).join('')||'<p>—</p>';}
document.getElementById('showHelp').addEventListener('click',()=>openInfoCard('Как пользоваться',CONTENT.help||''));
document.getElementById('showChangelog').addEventListener('click',()=>openInfoCard('Что нового',changelogHtml()));
document.getElementById('showFeedback').addEventListener('click',()=>openInfoCard('Обратная связь',CONTENT.feedback||''));
